...
public static class ShipPhotoUtil
{
    ...
    private static ValueTuple<Sprite, IEnumerable<ValueTuple<TurretHardpoint, Vector3>>> TakePhotoInner(Ship s, int ImgH, int ImgW, bool withTurretPos, Camera cam)
    {
        ...
        Vector3 shipAxis = s.transform.forward;
    ...
    private static float ShipLenCenter(ShipBase sb)
    {
        ...
        float localCenter = (m.bounds.max.z + m.bounds.min.z) / 2.0f;
        return localCenter * sb.HullObject.transform.lossyScale.z;
    }
    ...
    private static (Vector2, Vector2) ObjBoundsData(Transform t)
    {
        ...
        float minX = meshes[0].mesh.bounds.min.x * meshes[0].transform.lossyScale.x,
              ...
              minY = meshes[0].mesh.bounds.min.z * meshes[0].transform.lossyScale.z,
              maxY = meshes[0].mesh.bounds.max.z * meshes[0].transform.lossyScale.z;
        for (int i = 1; i < meshes.Length; ++i)
        {
            ...
            minY = Mathf.Min(minY, meshes[i].mesh.bounds.min.z * meshes[0].transform.lossyScale.z);
            maxY = Mathf.Max(maxY, meshes[i].mesh.bounds.max.z * meshes[0].transform.lossyScale.z);
        }
