// Downloaded from https://gist.github.com/StagPoint/e763b86ba617617f56b5a1e1dd69fd8c.
// I have no idea under what what license github gists are published.
// I modified the code to return items with their cost
...
namespace StagPoint.Collections
{
	...
	public class BinaryMinHeap<TData, TCost> where TCost : IComparable<TCost>
	{
		...
		#region Methods added by Boris
		/// <summary>
		/// Returns the item with the lowest cost without removing it from the heap
		/// </summary>
		/// <returns></returns>
		public (TData, TCost) PeekWithCost()
		{
			if (this.m_count == 0)
			{
				throw new InvalidOperationException("Cannot peek at first item, heap is empty.");
			...
			return (m_items[0].Data, m_items[0].Cost);
		}
		...
		/// </summary>
		/// <returns>The first value in the heap.</returns>
		public (TData, TCost) RemoveWithCost()
		{
			if (this.m_count == 0)
			{
				throw new InvalidOperationException("Cannot remove item, heap is empty.");
			...
			TData v = m_items[0].Data;
			TCost c = m_items[0].Cost;
